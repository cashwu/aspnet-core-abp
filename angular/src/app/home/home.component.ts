...
export class HomeComponent extends AppComponentBase implements AfterViewInit {
    ...
    ngAfterViewInit(): void {
        ...
        abp.auth.isGranted("admin");

        abp.log.info("test 123");

        var userLoginEnable = abp.setting.get("UserLoginEnable");
        abp.log.info(` user login enable :: ${userLoginEnable}`);

        abp.log.info(` clock now :: ${abp.clock.now()} `);
